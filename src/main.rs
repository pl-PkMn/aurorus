use std::env;
        ...
        mod display {
            ...
            pub fn print_package(index: usize, pkg: &AurPackage) {
                let installed = if pacman::is_installed(&pkg.name, false) { " (Installed)" } else { "" };
            ...
            pub fn print_official_pkg(index: usize, line: &str, description: Option<&str>) {
                if let Some(repo_start) = line.find('[') {
                    ...
                    if !parts.is_empty() {
                        ...
                        let installed = if pacman::is_installed(pkg_name, false) { " (Installed)" } else { "" };
        ...
        mod actions {
            ...
            pub async fn install_package(client: &Client, query: &str) -> Result<()> {
                ...
                let selection: usize = match input.parse() {
                    ...
                    _ => {
                        return Err(format!("Invalid selection. Please enter a number between 1 and {}.", total_count).into());
                    }
                };
